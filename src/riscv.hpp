...
class Visitor_ir {
  private:
    std::string register_name[16] = {"x0","t0","t1","t2","t3","t4","t5","t6","a0","a1","a2","a3","a4","a5","a6","a7"};
    ...
    // 为函数布置栈帧需要的量
    int var_space; // 需要为每个局部变量和临时符号分配的空间
    int ra_space; // 出现 call 则需要为原本的返回地址分配帧空间
    int param_space; // call 的函数超过 8 个参数，需要为其分配栈空间
